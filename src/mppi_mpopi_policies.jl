...
function sample_trajectories(H, x0, K, T; g_mean=nothing, σ_g=0.1, rng=Random.GLOBAL_RNG)
    ...
    # Sort indices by similarity of first state to x0
    idx_sorted = sortperm(H, by=dist_to_ist)

    # If K <= N take the nearest K, otherwise take nearest N then sample remaining indices with replacement
    if K <= N
        idx = idx_sorted[1:K]
    else
        # take all nearest first, then sample additional trajectories with replacement
        extra = rand(1:N, K - N)
        idx = vcat(idx_sorted, extra)
    end

    # Stack actions and states for vectorized selection
    # Assumes each h in H has shape (T x P) and that actions are in columns 4:5 and states in 1:3
    action_cols = 4:5
    state_cols = 1:3
    as = length(action_cols)
    ss = length(state_cols)

    # Preallocate stacked arrays: (T, as, N) and (T, ss, N)
    H_actions = Array{Float64}(undef, T, as, N)
    H_states  = Array{Float64}(undef, T, ss, N)
    for i in 1:N
        h = H[i]
        H_actions[:, :, i] = h[:, action_cols]
        H_states[:,  :, i] = h[:, state_cols]
    end

    # # Build one-hot selector matrix g (N x K)
    # g = zeros(Float64, N, K)
    # for k in 1:K
    #     g[idx[k], k] = 1.0
    # end
    ...
    # # kernel = exp.(-0.5 .* (dist ./ (maximum(dist) + eps())) )
    ...
    sampled_actions = [reshape(S_actions[:, k], T, as) for k in 1:K]

    Hmat_states = reshape(H_states, T * ss, N)     # (T*ss) x N
...
function calculate_trajectory_costs(pol::Data_Policy, env::AbstractEnv)
    # Use the number of stored trajectories in pol.H to define K so behavior
    # matches the operator which selects from the replay buffer.
    K, T = pol.params.num_samples, pol.params.horizon
