...
function simulate_deppi_car(; T_ini=20, N_pred=15, horizon=50, steps=300, num_samples=150, λ=10.0, α=1.0,
    ...
    for kk in 1:K
        ...
        if size(Ulog,1) >= T_ini
            U_p[:,kk] = vec(Ulog[1:T_ini, :]) else U_p[:,kk] = vec(vcat(Ulog, repeat(Ulog[end:end,:], T_ini-size(Ulog,1),1))) end
        if size(Ulog,1) >= N_pred
            U_f[:,kk] = vec(Ulog[end-N_pred+1:end, :]) else U_f[:,kk] = vec(vcat(repeat(Ulog[1:1,:], N_pred-size(Ulog,1),1), Ulog)) end
        if has_traj
            Ylog = pol.logger.trajectories[kk]
            if size(Ylog,1) >= T_ini
                Y_p[:,kk] = vec(Ylog[1:T_ini, :]) else Y_p[:,kk] = vec(vcat(Ylog, repeat(Ylog[end:end,:], T_ini-size(Ylog,1),1))) end
            if size(Ylog,1) >= N_pred
                Y_f[:,kk] = vec(Ylog[end-N_pred+1:end, :]) else Y_f[:,kk] = vec(vcat(repeat(Ylog[1:1,:], N_pred-size(Ylog,1),1), Ylog)) end
        else
    ...
    for t in 1:steps
        ...
        gc_costs[t] = calculate_trajectory_costs_deppi(U_p, Y_p, reshape(g_c, :, 1); u_past=u_past_vec, y_past=y_past_vec, pnorm=2, lambda_g=deepc_lambda_g)[1]
        if save_gif
            # Basic plot (trajectory plotting optional)
            plt = plot(env, text_output=text_with_plot, text_xy=text_on_plot_xy)
            frame(anim, plt)
        end
        env.done && break
    end
    ...
    if save_outputs
        isdir(hankel_dir) || mkpath(hankel_dir)
        writedlm(joinpath(hankel_dir, "$(hankel_prefix)_simple_W.csv"), W, ',')
        writedlm(joinpath(hankel_dir, "$(hankel_prefix)_simple_g_c_final.csv"), g_c, ',')
        writedlm(joinpath(hankel_dir, "$(hankel_prefix)_simple_g_c_hist.csv"), g_hist, ',')
        writedlm(joinpath(hankel_dir, "$(hankel_prefix)_simple_u_applied.csv"), u_applied, ',')
        open(joinpath(hankel_dir, "$(hankel_prefix)_simple_costs.csv"), "w") do io
            println(io, "step,g_c_residual_cost")
            for t in 1:steps
                println(io, string(t, ",", gc_costs[t]))
            end
    ...
    if save_gif && anim !== nothing
        gif_name = "$(hankel_prefix)_simple_deppi.gif"
